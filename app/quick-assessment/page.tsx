...
export default function QuickAssessment() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (trimmed) {
      setSubmittedCountry(trimmed); // store separately so results don't change as user types
      setSearched(true);
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <form onSubmit={handleSearch} className="flex gap-4">
          ...
          <button
            ...
            className="bg-spearfish hover:bg-[#0f6c7f] disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3.5 px-10 rounded-lg shadow-[0_4px_14px_rgba(18,135,159,0.3)] transition-all hover:scale-105 active:scale-95"
          >
            Analyze
          </button>
        </form>
          ...
          <div className="md:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 p-8 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h2 className="text-3xl font-extrabold text-slate-800 uppercase tracking-widest">{submittedCountry}</h2>
                  <p className="text-slate-500 flex items-center gap-1.5 mt-2 font-medium">
                    <MapPin size={16} /> Regional Security Assessment
                  </p>
                </div>
                ...
                <div className="flex gap-4 p-5 rounded-xl bg-slate-50 border border-slate-100">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 mt-2 leading-relaxed font-medium">
                      Elevated risk of civil unrest and localized political violence. Travel outside major urban centers is strongly discouraged without an armed escort. Increased vigilance required at border crossings.
                    </p>
                  </div>
                </div>
                <div className="flex gap-4 p-5 rounded-xl bg-slate-50 border border-slate-100">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 mt-2 leading-relaxed font-medium">
                      Maintain a low profile. Implement mandatory daily check-ins for all active personnel via the Journey Management module. Trigger immediate alerts for cross-region movements.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <a
              href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(submittedCountry)}`}
              target="_blank"
              rel="noreferrer"
            ...
            >
              ...
              View {submittedCountry} on Google Maps
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 flex flex-col">
            ...
            <div className="space-y-6 flex-1">
              <div>
                ...
                <div className="text-4xl font-black text-slate-800 flex items-baseline gap-1 mt-1">
                  84 <span className="text-base text-slate-400 font-semibold mb-1">/ 100</span>
                </div>
