...
export default function TravelPlansDashboard() {
  ...
  useEffect(() => {
    fetch('/api/travel-plans')
      ...
      .catch(err => {
        console.error('Travel plans fetch error:', err);
  ...
  const createNewPlan = async () => {
    ...
    try {
      ...
      router.push(`/travel-plans/${newPlan.id}`); // fixed: was window.location.href
    } catch (err) {
      console.error('Create travel plan error:', err);
      alert('Failed to create travel plan. Check your API route.');
    } finally {
      setCreating(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-10">
        ...
        <button
          ...
          disabled={creating}
          className="bg-spearfish hover:bg-[#0f6c7f] disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-3 px-6 rounded-lg shadow-md transition-all flex items-center gap-2"
        >
          <Plus size={20} /> {creating ? 'Creating...' : 'Create New Plan'}
        </button>
      </div>
          ...
          <div key={plan.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col">
            <div className="p-6 border-b border-slate-100 flex-1">
              <div className="flex justify-between items-start mb-4">
                <span className="text-xs font-black uppercase tracking-widest bg-slate-100 text-slate-600 px-2.5 py-1 rounded">
                  {plan.id}
                </span>
                <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded shadow-sm ${
                  (plan.riskStatus || '').includes('HIGH') || (plan.riskStatus || '').includes('EXTREME')
                    ? 'bg-red-500 text-white'
                    : (plan.riskStatus || '').includes('MEDIUM')
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-sm text-slate-600">
                  <MapPin size={16} className="text-spearfish" />
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-100 flex justify-between items-center">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                {plan.risks?.length || 0} Identified Risks
              </span>
              <Link
                href={`/travel-plans/${plan.id}`}
                className="text-spearfish hover:text-[#0f6c7f] font-bold text-sm flex items-center gap-1 group"
              >
